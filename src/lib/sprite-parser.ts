...
/**
 * Regex for sprite mob IDs: {race}_{class}_t{tier}
 * The ID may be zone-prefixed (e.g. "player_sprites:archae_bulwark_t10").
 * Tier is either a number or "staff".
 */
const SPRITE_ID_RE = /(?:^|:)([a-z]+)_([a-z]+)_t(\d+|staff)$/;
...
export function detectSpriteZone(entities: Entity[]): SpriteConfig | null {
  ...
  // Sort tiers by numeric value, with "tstaff" last
  const tierOrder = (t: string) => t === "tstaff" ? 9999 : parseInt(t.slice(1));
  const sortedTiers = [...tiers].sort((a, b) => tierOrder(a) - tierOrder(b));
